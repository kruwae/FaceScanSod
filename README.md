...
### ปัญหา Google Login ใช้ได้บางเครื่อง แต่บางเครื่องใช้ไม่ได้

อาการนี้มักเกิดจากการตั้งค่า browser หรืออุปกรณ์ที่บล็อกการทำงานของ Google Sign-In เช่น third-party cookies, popups, cross-site tracking หรือ privacy extension ไม่ได้เกิดจากระบบรหัสแอดมินเสมอไป

#### Google Chrome บน Mac
1. อัปเดต Chrome ที่ `chrome://settings/help`
2. ไปที่ `chrome://settings/cookies`
3. แนะนำให้เปิด `Allow all cookies` ระหว่างทดสอบ หรืออย่างน้อยปิด `Block third-party cookies`
4. ไปที่ `chrome://settings/content/popups`
5. เพิ่มเว็บจริงของคุณ เช่น `https://face-scan-sod.vercel.app` เป็น Allowed
6. ไปที่ `chrome://settings/content/javascript` แล้วตรวจว่า JavaScript เป็น Allowed
7. ปิด extension ที่อาจรบกวน เช่น AdBlock, uBlock, Ghostery, Privacy Badger
8. หากยังไม่ได้ ให้ลอง Incognito และล้าง cache/cookies

#### Google Chrome บน iPhone
> Chrome บน iPhone ใช้ engine เดียวกับ Safari จึงต้องดูการตั้งค่าของ Safari ด้วย

1. ไปที่ `Settings → Chrome`
2. เปิด `Allow Cross-Website Tracking`
3. ไปที่ `Settings → Safari`
4. ตั้ง `Block All Cookies = OFF`
5. ตั้ง `Prevent Cross-Site Tracking = OFF` ชั่วคราวเพื่อทดสอบ
6. ตั้ง `Block Pop-ups = OFF`

#### Safari บน Mac
1. เปิด `Safari → Settings → Advanced`
2. เปิด `Show Develop menu`
3. ไปที่ `Safari → Settings → Privacy`
4. ตั้ง `Block all cookies = OFF`
5. ตั้ง `Prevent cross-site tracking = OFF` ชั่วคราวเพื่อทดสอบ
6. ไปที่ `Safari → Settings → Websites → Pop-up Windows`
7. อนุญาตเว็บของคุณเป็น `Allow`
8. ใช้ `Develop → Empty Caches`

#### Safari บน iPhone
1. ไปที่ `Settings → Safari`
2. ตั้ง `Block All Cookies = OFF`
3. ตั้ง `Prevent Cross-Site Tracking = OFF` ชั่วคราวเพื่อทดสอบ
4. ตั้ง `Block Pop-ups = OFF`
5. หากยังไม่ได้ ให้ใช้ `Clear History and Website Data`

#### ตรวจ Google Cloud Console เพิ่มเติม
ใน OAuth Client ของ Google ให้ตรวจว่า `Authorized JavaScript origins` มีโดเมนจริงที่ใช้งานอยู่ เช่น:

```text
https://face-scan-sod.vercel.app
```

#### คำแนะนำเมื่อยังใช้งาน Google Login ไม่ได้
1. ใช้ `รหัสแอดมิน` แทนชั่วคราวเพื่อให้ระบบทำงานต่อได้
2. ทดสอบบน Chrome เวอร์ชันล่าสุดแบบไม่มี extension
3. อนุญาต cookies, popups และ cross-site tracking ตามรายการด้านบน
4. ล้าง cache/cookies แล้วลองใหม่
5. หลีกเลี่ยงการเปิดผ่าน in-app browser หรือ embedded browser
